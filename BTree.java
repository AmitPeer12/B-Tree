...
public class BTree<T extends Comparable<T>> {
    ...
    public T delete(T value) {
        Node<T> node = getNode_Combine(value);
        ...
        while (case4) {
            ...
            if (node.numberOfChildren() == 0) {
                ...
                if (lesser.numberOfKeys() > minKeySize) {
                    Node<T> predecessor = this.getGreatestNode(lesser);
                    T replaceValue = this.removeGreatestValue(predecessor);
                ...
                //case 3
                    ...
                    Node<T> successor = this.getSmallestNode(greater);
                    T replaceValue = this.removeSmallestValue(successor);
                    node.removeKey(value);
                    node.addKey(replaceValue);
                    case4 = false;
                }
    ...
    public boolean insert2pass(T value) {
        Node<T> toInsert = navigate(value);
        //an empty tree
        if (toInsert == null) {
            root = new Node<T>(null, maxKeySize, maxChildrenSize);
            root.addKey(value);
        }
        //there is an empty room in the leaf
        else if (toInsert.numberOfKeys() < maxKeySize) {
            toInsert.addKey(value);
        }
        //a full leaf
        else {
            Node<T> parent = toInsert.parent;
            while (parent != null && parent.numberOfKeys() == maxKeySize) {
                parent = parent.parent;
            }
            if (parent == null) {
                insert(value);
            } else {
                insert2pass(parent, value);
            }
        }
        return true;
    }

    private boolean insert2pass(Node<T> node, T value) {
        while (node != null) {
            if (node.numberOfKeys() == maxKeySize) {
                split(node);
                node = node.parent;
    ...
    private Node<T> navigate(T value) {
        Node<T> node = root;
        if (node == null) {
            return null;
        }
    ...
    private T removeSmallestValue(Node<T> toInsert) {
        T value = null;
        if (toInsert.numberOfKeys() > 0) {
            value = toInsert.removeKey(0);
        }
        return value;
    }
    ...
    private Node<T> getNode_Combine(T value) {
        Node<T> node = root;
        while (node != null) {
    ...
    private Node<T> merge(Node<T> curr, int index) {
        Node<T> leftChild = curr.getChild(index);
        Node<T> rightChild = curr.getChild(index + 1);
        T value = curr.removeKey(index);
        curr.removeChild(rightChild);
        leftChild.addKey(value);
        for (int i = 0; i < rightChild.keysSize; i++) {
            T v = rightChild.getKey(i);
            leftChild.addKey(v);
        }
        for (int i = 0; i < rightChild.childrenSize; i++) {
            Node<T> c = rightChild.getChild(i);
            leftChild.addChild(c);
        }
        return leftChild;
    }
    ...
    public static arr tar1(int n,int x){
            if (n<=0){
